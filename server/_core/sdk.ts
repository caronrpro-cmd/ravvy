...
class SDKServer {
  ...
  private getSessionSecret() {
    // Must match the fallback used in localAuth.ts signToken()
    const secret = ENV.cookieSecret || "local-dev-secret-please-set-JWT_SECRET-in-env";
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // If user not in DB, sync from OAuth server — but only for OAuth tokens.
    // Local tokens (openId starts with "local:") are self-contained: the user
    // is always created in DB by localAuth at registration time, so reaching
    // this branch with a local openId means the account was deleted.
    if (!user) {
